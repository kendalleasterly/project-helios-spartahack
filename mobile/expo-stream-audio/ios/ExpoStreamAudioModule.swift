...
public class ExpoStreamAudioModule: Module {
  ...
  private func startRecordingInternal(options: [String: Any]) async throws {
    do {
      // voiceChat mode requires playAndRecord; .record can trigger OSStatus -50.
      ...
      let fallbackMessage = makeAudioSessionErrorMessage(
        stage: "setCategory(playAndRecord, voiceChat)",
        error: error
      )
      sendError(fallbackMessage)
      ...
      do {
        ...
        let message = makeAudioSessionErrorMessage(
          stage: "setCategory(playAndRecord, default)",
          error: error
        )
        sendError(message)
        throw NSError(
    ...
    if let requestedSampleRate = options["sampleRate"] as? Double {
      do {
        ...
        let message = makeAudioSessionErrorMessage(
          stage: "setPreferredSampleRate(\(requestedSampleRate))",
          error: error
        )
        sendError(message)
      }
  ...
  private func makeAudioSessionErrorMessage(stage: String, error: Error) -> String {
    ...
    return "Failed to configure audio session (\(details)): \(nsError.localizedDescription)"
  }
